...
// line is a shorthand for a norm.Line without a branch.
func line(text string) norm.Line { return norm.Line{Text: text, Target: -1} }

// branch is a shorthand for a branch line; the label slot uses the
// same internal marker as norm.NormalizeLines ("\x01").
func branch(op string, target int) norm.Line {
	return norm.Line{Text: op + " \x01", Target: target}
}
